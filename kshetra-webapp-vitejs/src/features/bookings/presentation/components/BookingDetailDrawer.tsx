...
export function BookingDetailDrawer({
  ...
  return (
    <>
      {/* This is the app's one true slide-over: the list stays visible beside
          it, so "outside" is a real place to click. The scrim is transparent
          rather than dimmed — it is here to catch the click and to stop it
          landing on a row underneath, which would otherwise swap the panel's
          contents instead of closing it. */}
      <div aria-hidden onClick={onClose} className="fixed inset-0 z-drawer" />
      ...
      <div className="fixed inset-y-0 right-0 z-drawer flex w-full flex-col bg-sunken shadow-xl sm:w-[560px]">
        <div className="flex h-14 flex-shrink-0 items-center gap-2.5 border-b border-stroke bg-card px-6">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <span className="text-xs font-semibold uppercase tracking-overline text-ink-subtle">
              Pooja Bookings
            </span>
          ...
          <div className="mx-auto flex max-w-[880px] flex-col gap-4 p-6 pb-14">
            {booking.isOverdue && (
              <Alert type="warning" icon={<Icon name="clock-countdown" size={16} />}>
                Past its completion window. Nothing expires on its own — reassign it or record it as
                performed.
              </Alert>
            )}
            ...
            <div className="flex flex-col gap-3.25 rounded-2xl bg-card p-5 shadow-sm">
              ...
              <DetailRow label="Pooja date">
                <span className="tabular-nums">
                  {booking.poojaDate ? formatFullDate(booking.poojaDate) : '—'}
                </span>
              </DetailRow>
            ...
            {showActions && (
              <div className="flex flex-col gap-3.25 rounded-2xl bg-card p-5 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {canComplete && completable && (
                    <Button
                      theme="primary"
                      disabled={busy}
                      iconLeft={<Icon name="seal-check" size={16} />}
                ...
                <div className="flex items-center gap-1.75 text-xs text-ink-subtle">
                  <Icon name="clock-countdown" size={14} />A reassigned pooja must be completed
                  within 24 hours.
                </div>
              </div>
            )}
