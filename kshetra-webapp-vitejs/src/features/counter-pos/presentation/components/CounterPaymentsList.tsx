...
import { Badge, Icon, Input, Spinner } from '@/shared/ui'
...
export function CounterPaymentsList({ search, onSearchChange, rows, isLoading, errorMessage, onSelect }: CounterPaymentsListProps) {
  return (
    <>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {!isLoading && !errorMessage && rows.map((r) => (
          <button
            key={r.orderId}
            type="button"
        ...
        {isLoading && (
          <div className="flex items-center justify-center gap-2.5 px-8.5 py-8.5 text-sm text-ink-subtle">
            <Spinner size={18} />
            Loading bookings…
          </div>
        )}
        {!isLoading && errorMessage && <div className="px-8.5 py-8.5 text-center text-sm text-danger">{errorMessage}</div>}
        {!isLoading && !errorMessage && rows.length === 0 && (
          <div className="px-8.5 py-8.5 text-center text-sm text-ink-subtle">No app agent-code bookings match.</div>
        )}
      </div>
    </>
  )
