...
export interface DrawerHeaderProps {
  ...
  isView: boolean
  saveLabel: string
  /** A write is in flight — the actions lock so a double-click cannot submit twice. */
  busy?: boolean
  onBack: () => void
  onEdit: () => void
  onCancel: () => void
  ...
  return (
    <div className="flex h-14 shrink-0 items-center gap-2.5 border-b border-stroke bg-card px-6">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <span className="truncate whitespace-nowrap text-base font-semibold text-ink-strong">
          {title}
        </span>
      </div>
      {isView ? (
      ...
      {isView && (
        <Button
          ...
          size="sm"
          onClick={onEdit}
          iconLeft={<Icon name="pencil-simple" size={14} />}
        >
          Edit
        </Button>
      )}
      {!isView && (
        <>
          <Button theme="default" variant="outline" size="sm" onClick={onCancel} disabled={busy}>
            Cancel
          </Button>
          <Button theme="primary" size="sm" onClick={onSave} disabled={busy}>
            {saveLabel}
          </Button>
        </>
      )}
